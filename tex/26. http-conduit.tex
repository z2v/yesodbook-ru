...
\begin{itemize}
\item A new connection Manager is allocated.
\item The URL is parsed to a Request. If the URL is invalid, then an
  exception is thrown.
\item The HTTP request is made, following any redirects from the server.
\item If the response has a status code outside the 200-range, an exception is thrown.
\item The response body is read into memory and returned.
\item runResourceT is called, which will free up any resources (e.g., the open
