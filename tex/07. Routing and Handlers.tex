...
представляющих файловую структуру \marginpar{file structure - Может, имеется в виду
  структура файловой системы?} или вики с произвольными иерархиями.
...
приложения будет просто \lstinline!/!. Также, вы возможно захотите поместить свои
часто задаваемые вопросы (FAQ) по адресу \lstinline!/page/faq!.
...
\lstinline!/fib/#Int!. Но тут есть следующая небольшая проблема: мы не хотим
разрешать, чтобы отрицательные числа или ноль передавались в наше
приложение. К счастью, система типов может защитить нас:
\begin{lstlisting}
