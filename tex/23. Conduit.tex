...
% Convenience Exports
% 
% In addition to what's been listed so far, there are a few extra functions exported
% (mostly) for
% convenience.
%   
...
% main = return ()
% \end{lstlisting}
% These sources are very straight-forward, since they always return the
% same results.
...
% the current state, and it returns a new state and a return value. Let's use this to
% reimplement
% sourceList.
% \begin{lstlisting}
%   -- START
...
% Sinks
% 
% A sink consumes a stream of data, and produces a result. A sink must always produce a
% result,
...
% together into a larger sink. You can also use the built-in sink functions to perform most
% of your
...
% arguments: an initial state, a push function (takes the current state and next input, and
% returns
% a new state and result) and a close function (takes the current state and returns an
% output). As
...
% that there is no leftover input (we'll discuss that later). It also gives an output of all
% the
% characters it has received. The second clause simply appends the new character to the
% existing
...
% standard code to pull lines from standard input. This familiar interface should make it
% easy to
...
% List functions
% 
% As easy as it is to write your own sinks, you'll likely want to take advantage of the
% built-in
...
% 
% If you're looking for some way to practice with conduits, reimplementing the functions in
% the
...
% simpler, it's less error prone, and means you have a lot of flexibility in where your data
% is
...
% 
% However, there is one thing I wanted to point out from the long-winded example. On the
% second
...
% away the leftovers. When we cover resumable sources later, we'll see that the leftover
% value is
% put back on the buffer to allow later sinks reusing an existing source to pull the value.
% 
% Conduit
% 
% This section covers the final major datatype in our package, conduits. While sources
% produce a
...
%     }
% \end{lstlisting}
% This should look very similar to what we've seen with sinks. A conduit can be pushed to,
% in
% which case it returns a result. A result either indicates that it is still producing data,
% or
...
% because a conduit will product a new stream of output instead of producing a single output
% value
...
% \end{lstlisting}
%  Stateful conduits
%   Of course, not all conduits can be declared without state. Doing so on the bare metal is
% not
%    too difficult.
% \begin{lstlisting}
%   -- START
...
% 
% It may seem like all these different approaches are redundant. While occassionally you can
% in
...
%     go x = [show x]
% \end{lstlisting}
% We no longer have the 1-to-1 correspondence. As a result, we can't use the second method.
% But
