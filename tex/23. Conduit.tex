...
% simpler, it's less error prone, and means you have a lot of flexibility in where your
data
% is
...
% sink to the next in a binding. And in fact, the real connect operator doesn't always
throw
% away the leftovers. When we cover resumable sources later, we'll see that the leftover
% value is
% put back on the buffer to allow later sinks reusing an existing source to pull the
value.
% 
% Conduit
% 
...
% in
% which case it returns a result. A result either indicates that it is still producing
data,
% or
