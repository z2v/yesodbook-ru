...
Возьмём, например, енумераторы (пакет \verb{enumerator}). Если вы заглянете в документацию, то найдёте функцию
...
\textit{Операции в монаде \lstinline{ST} не безопасны относительно исключений. Вы не должны выделять
...
воспользуемся некоторыми функциями из пакета \verb{monad-control}
...
данные, если он уже вернул \verb{Closed}. Чтобы облегчить работу с буферизованными 
